...
import { getDemoConversations } from '@/lib/demo-content';

export default async function MessagesPage() {
  const conversations = await getDemoConversations();
        ...
        {activeConversation ? (
          <>
            <div className="flex items-center gap-3 border-b border-border pb-4">
              ...
              <div>
                <p className="text-lg font-semibold">{activeConversation.title}</p>
                <p className="text-sm text-muted-foreground">{activeConversation.meta}</p>
              </div>
            </div>

            <div className="space-y-4 py-6">
              {activeConversation.messages.map((message, index) => (
                <Bubble key={`${activeConversation.id}-${index}`} author={message.author} text={message.text} mine={Boolean(message.mine)} />
              ))}
            </div>
          </>
        ) : null}
